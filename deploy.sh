...
flash_keyboard() {
    local firmware=$1 deadline mount_dir info current_serial
...
main() {
    local script script_dir firmware
    ...
    if [[ $# == 0 ]]; then
        ...
        exec sudo "$BASH" "$script" --flash "$port" "$serial"
    elif [[ $# == 3 && "$1" == '--flash' && $EUID == 0 ]]; then
        port=$2
        serial=$3
        flash_keyboard "$firmware"
    else
        echo 'Usage: ./deploy.sh (start with one Corne half connected in normal mode)' >&2
        return 1
    fi
}
